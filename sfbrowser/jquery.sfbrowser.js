...
*		- added and tested with Tinysort 1.1.1
*
* in last update:
...
;(function($) {
	...
	$.sfbrowser = {
		 id: "SFBrowser"
		,version: '3.4.62'
		,copyright: "Copyright (c) 2007 - 2012 Ron Valstar"
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function closeSFB() {
		trace("sfb close");
		// todo: confirm to remove all pending uploads
	...
	function setupDropBox() {
		$Dropbox.find('>span').text('Drop here...');
		//
		var bDropboxVisible = false;
		function toggleDropBox(b){
			if (b&&!bDropboxVisible){
				$Dropbox.removeClass('hover');
				$Dropbox.css({display:'table'});
				bDropboxVisible = true;
			} else if (!b&&bDropboxVisible){
				$Dropbox.css({display:'none'});
				bDropboxVisible = false;
			}
		}
		//
		$Window.bind('dragenter dragleave mouseup',function handleWindow(e){
			e.preventDefault();
			switch (e.type) {
				case 'dragenter': toggleDropBox(true); break;
				case 'dragleave':
		...
		$Dropbox.bind('dragenter dragover drop dragleave',function(e){
			console.log('handleDropBox',e.type); // log
			switch (e.type) {
				case 'dragenter':
				case 'dragover':
					e.preventDefault();
			...
			switch (e.type) {
				...
				case 'drop':
					console.log('handleDropBox',e.type,e.originalEvent.dataTransfer.files.length||-1,e); // log
					toggleDropBox(false);
					var oDataTransfer = e.originalEvent.dataTransfer;
					var oFiles = oDataTransfer.files||oDataTransfer.getData();
	...
	function keyDown(e) {
		...
		// todo: fix: renameTryToPost doet rename disabelen
		...
		switch (e.keyCode) {
			case 13: if (bIsOpen) chooseFile(); break; // todo: disable in pluginmode
		}
...
//			if (bOverlay) closeSFB();
	...
	function renameTryToPost() { // todo: first check filename clientside (saves a post)
	...
	function fileUpload(aFiles) {
		$.each(aFiles,function(i,file){
			...
			// cancel upload
			$TrUp.find('a.cancel').click(function(e){
				oUpload.abort();
				$TrUp.remove();
			});
		});
...
// todo: fucking IE forces prototype... oh well...
...
if (!window.UP) {
	window.UP = function(){
		...
		function upload(file,vars) {
			...
			/*function getXHR(openvars,setRequestHeader) {
				...
				oReturn.xhr = oXHR;
				if (openvars!==undefined) oXHR.open('POST', oUp.to+varsToString(openvars), true);
				if (setRequestHeader!==undefined) oXHR.setRequestHeader('content-type', 'multipart/form-data; boundary=' + sBoundary);
				return oXHR;
			}*/
			var oXHR = new XMLHttpRequest();
			oXHR.onreadystatechange = function(e) {
				if (oXHR.readyState!==4) return;
				oUploads.remove(oReturn);
				fnDone&&fnDone.apply(this,[oXHR.response||oXHR.responseText]);
			};
			oReturn.xhr = oXHR;
			oReturn.abort = function(){
				oXHR.abort();
			};
...
//							if (fnDone) fnDone.apply(this,[oFileReader]);
						break;
						case 'error': // todo: add error response
